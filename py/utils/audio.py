...
def openAudioFile(path, sample_rate=32000, as_mono=True, mean_substract=False):
...
def melspec(sig, rate, shape=(256, 512), fmin=50, fmax=32000, normalize=True, preemphasis=0.95):
...
def stft(sig, rate, shape=(256, 512), fmin=50, fmax=32000, normalize=True):
...
if __name__ == '__main__':
    ...
    for spec in specsFromFile('../example/Common Redpoll.mp3',
                              rate=44100,
                              seconds=3,
                              overlap=2,
                              minlen=1,
